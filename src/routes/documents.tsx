...
import { isChildViewer } from "@/lib/family/access";
...
function DocumentsPage() {
  ...
  const childView = isChildViewer(members, settings.currentMemberId);
  ...
  const list = documents.filter((d) => {
    if (childView && d.parentOnly) return false;
      ...
      {list.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {list.map((d) => {
            ...
            const groupMeta =
              GROUPS.find((g) => (g.cats as readonly string[]).includes(d.categoryId)) ??
                ...
                <div className="mt-3 flex gap-2">
                  {!childView ? (
                    <button
                      type="button"
                      onClick={() => {
                        updateDocument(d.id, { parentOnly: !d.parentOnly });
                        toast.success(
                          d.parentOnly
                            ? "Document visible pour tous"
                            : "Verrouillé pour les enfants",
                        );
                      }}
                      className="inline-flex h-10 items-center gap-1 rounded-full bg-surface-2 px-3 text-xs font-extrabold"
                    >
                      {d.parentOnly ? "🔒" : "🔓"}
                      {d.parentOnly ? "Parents" : "Tous"}
                    </button>
                  ) : null}
                  <button
                    type="button"
