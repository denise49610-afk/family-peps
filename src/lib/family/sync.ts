...
export async function createFamilyCloud(): Promise<{ code: string } | { error: string }> {
  try {
    const existing = normalizeFamilyCode(useFamilyStore.getState().settings.familyCode || "");
    if (existing) {
      useFamilyStore.getState().updateSettings({ familyCode: existing, cloudSync: true });
      try { localStorage.setItem("famizen-family-code", existing); } catch { /* */ }
      await startSync(existing);
      void pushNow(existing);
    ...
    try { localStorage.setItem("famizen-family-code", res.code); } catch { /* */ }
...
export async function joinFamilyCloud(
  ...
  try {
    ...
    if (!res.found) return { error: "Code introuvable — vérifiez ZEN-XXXXXX" };
    ...
    try {
      ...
      lastRemoteStamp = res.updatedAt;
      try { localStorage.setItem("famizen-family-code", code); } catch { /* */ }
    } finally {
    ...
    if (/unique|duplicate|already exist/i.test(msg)) {
      useFamilyStore.getState().updateSettings({ familyCode: code, cloudSync: true });
      try { localStorage.setItem("famizen-family-code", code); } catch { /* */ }
      await startSync(code);
      return { ok: true };
...
export async function leaveFamilyCloud() {
  ...
  try { localStorage.removeItem("famizen-family-code"); } catch { /* */ }
  lastRemoteStamp = "";
  setStatus("off");
}
